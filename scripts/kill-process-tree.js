...
export async function killProcessTree(pid, signal = "SIGTERM") {
  const rootPid = parseInt(pid, 10);
  // Upstream's fix for their #31: a non-numeric pid reaching `process.kill` is far worse than an
  // error, since `kill(0)` and negative values address process groups.
  if (Number.isNaN(rootPid)) throw new Error("pid must be a number");
  ...
  for (const treePid of await collectTree(rootPid)) signalPid(treePid, signal);
}
