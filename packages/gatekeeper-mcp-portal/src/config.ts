...
import { toolBelongsToServer } from "@gadgets/mcp-shared/portal";
...
export function requirePortalServerScope(
  ...
  for (const name of scope.tools ?? []) {
    ...
    if (!toolBelongsToServer(name, scope.serverId)) {
...
export function toolGrantOptions(args: {
  ...
  return args.tools.slice(0, MAX_TOOLS_PER_SERVER).map(tool => {
    return {
      value: tool.name,
      ...
      meta: classifyAnnotations(tool.annotations, args.trust).mode === "read"
        ? "read-only"
        : "needs approval",
    };
  });
