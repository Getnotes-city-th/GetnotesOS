...
export class McpClient {
  ...
  async #list<T extends { name: string }>(
    ...
    for (let page = 0; page < MAX_TOOL_PAGES; page++) {
      if (this.#discoveryPages >= MAX_TOOL_PAGES) return scanLimit();
      ...
      this.#discoveryPages++;
      this.#discoveryBytes += measured.responseBytes;
      if (this.#discoveryBytes > MAX_SCANNED_TOOL_BYTES) return scanLimit();
      const pageTools = body.tools ?? [];
      const remainingTools = Math.max(0, MAX_SCANNED_TOOLS - this.#discoveryTools);
      const scanCount = Math.min(pageTools.length, remainingTools);
      this.#discoveryTools += scanCount;
