...
// ---------------------------------------------------------------------------
{
  ...
  // For local testing, configure admin accounts.
  config.vars = config.vars || {};
  config.vars.ADMINS = config.vars.ADMINS ?? ["admin", "getnotesos"];
  if (!config.vars.ADMINS.includes("getnotesos")) {
    config.vars.ADMINS.push("getnotesos");
  }
  if (!config.vars.ADMINS.includes("admin")) {
    config.vars.ADMINS.push("admin");
  }
...
// this process group with the terminal attached, so Ctrl-C reaches it as before. GetnotesOS also
// restarts Wrangler after an unexpected exit while leaving the already-running UI watchers intact.
function startWrangler() {
  const child = spawn(wranglerCommand, wranglerArgv, { stdio: "inherit", cwd: ROOT });
  wranglerChild = child;
  ...
  child.on("close", async (code, signal) => {
    if (child !== wranglerChild) return;
    if (stoppingDevWatchers || shutdownExitCode !== null) {
      await stopDevWatchersDeep();
      process.exit(shutdownExitCode ??
        (signal ? 128 + (constants.signals[signal] ?? 0) : code ?? 1));
    }

    console.warn(
      `[run-dev-server] wrangler dev exited (status=${code}, signal=${signal}); ` +
      "auto-restarting in 1s...",
    );
    setTimeout(() => {
      if (!stoppingDevWatchers) startWrangler();
    }, 1_000);
  });
