...
async function stopDevWatchersDeep() {
  ...
  await Promise.all(devWatchers
      .filter(watcher => watcher.exitCode === null && watcher.pid)
...
// already tearing down its workerd children and exiting first would orphan them. SIGTERM is not
// group-delivered, so it has to be forwarded.
process.on("SIGINT", async () => {
  await stopDevWatchersDeep();
  // Registering this listener disables Node's own exit-on-SIGINT, so exit here when there is no
  // Wrangler left to wait for.
  if (wranglerChild?.exitCode !== null) process.exit(130);
});
process.on("SIGTERM", async () => {
  // Awaited before Wrangler is signalled: this process stays alive waiting for Wrangler's exit, so
  // there is time for the walk, and nothing is competing to exit underneath it.
  await stopDevWatchersDeep();
  if (wranglerChild?.exitCode === null) wranglerChild.kill("SIGTERM");
  else process.exit(143);
});

// The pre-flight builds still running. Kept separate from `devWatchers`, whose `exit` handler runs on
// every shutdown path; these need killing on one path only, when a sibling build has failed.
...
// which ones declare the task or script, matching the directory probes in the watcher loop below --
// so a new gatekeeper needs its `build:configurator` script or `build:app` task to be built here.
//
...
try {
  ...
  // immediate re-run. Two of the three are `pnpm exec vp run ...`, so this has to reach each
  // sibling's `vp`/`vite` descendants -- a bare kill() signals only the wrapper and leaves them
  // writing. Awaited: process.exit() would cut the tree walk short.
...
function withoutPnpmIndirection(pkgDir, command, depth = 0) {
  ...
  return command.replaceAll(/\bpnpm exec ([\w@/.-]+)/g, (original, bin) => {
    ...
    return entry ? `${JSON.stringify(process.execPath)} ${JSON.stringify(entry)}` : original;
  });
}
...
wranglerChild.on("exit", async (code, signal) => {
  ...
  await stopDevWatchersDeep();
  // The output was already shown via stdio: "inherit", so just propagate the exit status.
